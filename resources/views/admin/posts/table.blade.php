...
<div class="card-header">
  ...
  <div class="card-tools"><button class="btn btn-primary "> <a href="{{route('posts.create')}}" class="text-white">Thêm mới</a></button></div>
</div>
  <!-- /.card-header -->
  <div class="card-body">
    <div id="example2_wrapper" class="dataTables_wrapper dt-bootstrap4">
      ...
      <tr class="odd">  
        ...
        <td>{{$item->content}}</td>
        <td class="d-flex">
          <form action="{{ route('posts.edit', ['category' => $item->id]) }}" class="ms-2" method="GET">
            @csrf
            <button class="btn btn-primary ">Sửa tin</button>

          </form>
          <form action="{{ route('posts.destroy', ['category' => $item->id]) }}" method="POST">
            @csrf
            @method('DELETE')
